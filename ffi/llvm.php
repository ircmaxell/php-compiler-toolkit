...
';
 ...
 // typedefenum LLVMOpcode
    ...
    const LLVMBr = 2;
    const LLVMSwitch = 3;
    const LLVMIndirectBr = 4;
    const LLVMInvoke = 5;
    const LLVMUnreachable = 7;
    const LLVMAdd = 8;
    const LLVMFAdd = 9;
    const LLVMSub = 10;
    const LLVMFSub = 11;
    ...
    const LLVMCall = 45;
    const LLVMSelect = 46;
    const LLVMUserOp1 = 47;
    const LLVMUserOp2 = 48;
    const LLVMVAArg = 49;
    const LLVMExtractElement = 50;
    const LLVMInsertElement = 51;
    const LLVMShuffleVector = 52;
    const LLVMExtractValue = 53;
    const LLVMInsertValue = 54;
    const LLVMFence = 55;
    const LLVMAtomicCmpXchg = 56;
    const LLVMAtomicRMW = 57;
    const LLVMResume = 58;
    const LLVMLandingPad = 59;
    const LLVMCleanupRet = 61;
    const LLVMCatchRet = 62;
    const LLVMCatchPad = 63;
    const LLVMCleanupPad = 64;
    const LLVMCatchSwitch = 65;
 // typedefenum LLVMTypeKind
    const LLVMVoidTypeKind = 0;
    const LLVMHalfTypeKind = 1;
    const LLVMFloatTypeKind = 2;
    const LLVMDoubleTypeKind = 3;
    const LLVMX86_FP80TypeKind = 4;
    const LLVMFP128TypeKind = 5;
    const LLVMPPC_FP128TypeKind = 6;
    const LLVMLabelTypeKind = 7;
    const LLVMIntegerTypeKind = 8;
    const LLVMFunctionTypeKind = 9;
    const LLVMStructTypeKind = 10;
    const LLVMArrayTypeKind = 11;
    const LLVMPointerTypeKind = 12;
    const LLVMVectorTypeKind = 13;
    const LLVMMetadataTypeKind = 14;
    const LLVMX86_MMXTypeKind = 15;
    const LLVMTokenTypeKind = 16;
 // typedefenum LLVMLinkage
    const LLVMExternalLinkage = 0;
    const LLVMAvailableExternallyLinkage = 1;
    const LLVMLinkOnceAnyLinkage = 2;
    const LLVMLinkOnceODRLinkage = 3;
    const LLVMLinkOnceODRAutoHideLinkage = 4;
    const LLVMWeakAnyLinkage = 5;
    const LLVMWeakODRLinkage = 6;
    const LLVMAppendingLinkage = 7;
    const LLVMInternalLinkage = 8;
    const LLVMPrivateLinkage = 9;
    const LLVMDLLImportLinkage = 10;
    const LLVMDLLExportLinkage = 11;
    const LLVMExternalWeakLinkage = 12;
    const LLVMGhostLinkage = 13;
    const LLVMCommonLinkage = 14;
    const LLVMLinkerPrivateLinkage = 15;
    const LLVMLinkerPrivateWeakLinkage = 16;
 // typedefenum LLVMVisibility
    const LLVMDefaultVisibility = 0;
    const LLVMHiddenVisibility = 1;
    const LLVMProtectedVisibility = 2;
 // typedefenum LLVMDLLStorageClass
    const LLVMDefaultStorageClass = 0;
    const LLVMDLLImportStorageClass = 1;
    const LLVMDLLExportStorageClass = 2;
 // typedefenum LLVMCallConv
    const LLVMCCallConv = 0;
    const LLVMFastCallConv = 8;
    const LLVMColdCallConv = 9;
    const LLVMWebKitJSCallConv = 12;
    const LLVMAnyRegCallConv = 13;
    const LLVMX86StdcallCallConv = 64;
    const LLVMX86FastcallCallConv = 65;
 // typedefenum LLVMValueKind
    const LLVMArgumentValueKind = 0;
    const LLVMBasicBlockValueKind = 1;
    const LLVMMemoryUseValueKind = 2;
    const LLVMMemoryDefValueKind = 3;
    const LLVMMemoryPhiValueKind = 4;
    const LLVMFunctionValueKind = 5;
    const LLVMGlobalAliasValueKind = 6;
    const LLVMGlobalIFuncValueKind = 7;
    const LLVMGlobalVariableValueKind = 8;
    const LLVMBlockAddressValueKind = 9;
    const LLVMConstantExprValueKind = 10;
    const LLVMConstantArrayValueKind = 11;
    const LLVMConstantStructValueKind = 12;
    const LLVMConstantVectorValueKind = 13;
    const LLVMUndefValueValueKind = 14;
    const LLVMConstantAggregateZeroValueKind = 15;
    const LLVMConstantDataArrayValueKind = 16;
    const LLVMConstantDataVectorValueKind = 17;
    const LLVMConstantIntValueKind = 18;
    const LLVMConstantFPValueKind = 19;
    const LLVMConstantPointerNullValueKind = 20;
    const LLVMConstantTokenNoneValueKind = 21;
    const LLVMMetadataAsValueValueKind = 22;
    const LLVMInlineAsmValueKind = 23;
 ...
 // typedefenum LLVMRealPredicate
    ...
    const LLVMRealUNO = 8;
    const LLVMRealUEQ = 9;
    const LLVMRealUGT = 10;
    const LLVMRealUGE = 11;
    const LLVMRealULT = 12;
    const LLVMRealULE = 13;
    const LLVMRealUNE = 14;
    const LLVMRealPredicateTrue = 15;
 // typedefenum LLVMLandingPadClauseTy
    const LLVMLandingPadCatch = 0;
    const LLVMLandingPadFilter = 1;
 // typedefenum LLVMThreadLocalMode
    const LLVMNotThreadLocal = 0;
    const LLVMGeneralDynamicTLSModel = 0;
    const LLVMLocalDynamicTLSModel = 1;
    const LLVMInitialExecTLSModel = 2;
    const LLVMLocalExecTLSModel = 3;
 // typedefenum LLVMAtomicOrdering
    const LLVMAtomicOrderingNotAtomic = 0;
    const LLVMAtomicOrderingUnordered = 1;
    const LLVMAtomicOrderingMonotonic = 2;
    const LLVMAtomicOrderingAcquire = 4;
    const LLVMAtomicOrderingRelease = 5;
    const LLVMAtomicOrderingAcquireRelease = 6;
    const LLVMAtomicOrderingSequentiallyConsistent = 7;
 // typedefenum LLVMAtomicRMWBinOp
    const LLVMAtomicRMWBinOpXchg = 0;
    const LLVMAtomicRMWBinOpAdd = 1;
    const LLVMAtomicRMWBinOpSub = 2;
    const LLVMAtomicRMWBinOpAnd = 3;
    const LLVMAtomicRMWBinOpNand = 4;
    const LLVMAtomicRMWBinOpOr = 5;
    const LLVMAtomicRMWBinOpXor = 6;
    const LLVMAtomicRMWBinOpMax = 7;
    const LLVMAtomicRMWBinOpMin = 8;
    const LLVMAtomicRMWBinOpUMax = 9;
    const LLVMAtomicRMWBinOpUMin = 10;
 // typedefenum LLVMDiagnosticSeverity
    const LLVMDSError = 0;
    const LLVMDSWarning = 1;
    const LLVMDSRemark = 2;
    const LLVMDSNote = 3;
    const LLVMAttributeReturnIndex = 0;
    ...
    // enum LLVMByteOrdering
    const LLVMBigEndian = 0;
    const LLVMLittleEndian = 1;
 ...
 // typedefenum LLVMCodeModel
    ...
    const LLVMCodeModelSmall = 2;
    const LLVMCodeModelKernel = 3;
    const LLVMCodeModelMedium = 4;
    const LLVMCodeModelLarge = 5;
